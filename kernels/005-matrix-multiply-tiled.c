...
matrix_multiply_tiled(float* C, const float* A, const float* B, int N, int M,
                      int K) {
  ...
  for (int tile = 0; tile < (M + TILE_SIZE - 1) / TILE_SIZE; tile++) {
    ...
    if (tiled_row < N &&
        tile * TILE_SIZE + __builtin_amdgcn_workitem_id_x() < M) {
      tile_A[__builtin_amdgcn_workitem_id_y()]
            [__builtin_amdgcn_workitem_id_x()] =
                A[tiled_row * M +
                  (tile * TILE_SIZE + __builtin_amdgcn_workitem_id_x())];
    } else {
      tile_A[__builtin_amdgcn_workitem_id_y()]
            [__builtin_amdgcn_workitem_id_x()] = 0.0f;
    }

    // Load B tile from global memory
    if (tiled_col < K &&
        tile * TILE_SIZE + __builtin_amdgcn_workitem_id_y() < M) {
      tile_B[__builtin_amdgcn_workitem_id_y()]
            [__builtin_amdgcn_workitem_id_x()] =
                B[(tile * TILE_SIZE + __builtin_amdgcn_workitem_id_y()) * K +
                  tiled_col];
    } else {
      tile_B[__builtin_amdgcn_workitem_id_y()]
            [__builtin_amdgcn_workitem_id_x()] = 0.0f;
